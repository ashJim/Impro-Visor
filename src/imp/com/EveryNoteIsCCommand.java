...
/**
 * A Command that takes every note in a Part and turns them all into the note C.
 * Test part to figure out functionality of TRMs.
 * @see         Command
 * @see         CommandManager
 * @see         Part
 * @see         CopyCommand
 * @see         CutCommand
 * @author      Jim Ashford, adapted from code by Stephen Jones
 */
public class EveryNoteIsCCommand implements Command, Constants {
    ...
    /**
     * Creates a new Command that turn every note of a source part into C and
     * copy the result to a destination Part.
     ...
     */
    public EveryNoteIsCCommand(MelodyPart source, int startSlot, int stopSlot, boolean play) {
        this.source = source;
        this.dest = source;
     ...
     */
    public EveryNoteIsCCommand(MelodyPart source, int startSlot, int stopSlot,
                                     boolean undoable, boolean play) {
        this(source, startSlot, stopSlot, play);
        this.undoable = undoable;
        Trace.log(2, "creating EveryNoteIsC Command");
    }
      ...
      if( source != null && dest != null && source.size() > 0 )
      {
        ...
        if( lastIndex >= 0  ) {
          ...
          if(lastUnit instanceof Note) {
              ...
              if(stopIndex < dest.size() && dest.getNextIndex(lastIndex) > stopIndex) {
