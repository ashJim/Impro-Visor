...
/**
 * Test command to figure out how to implement ChordPart manipulation in a TRM.
 * @see         Command
 * @see         CommandManager
 * @see         Part
 * @see         CopyCommand
 ...
 */
public class ChordifyCommand implements Command, Constants {

    /**
     * the Part to paste over
     */
    protected ChordPart source;

    /**
     * the Part to paste onto
     */
    protected ChordPart dest;
     ...
     */
    public ChordifyCommand(ChordPart source, int startSlot, int stopSlot, boolean play) {
        this.source = source;
        this.dest = source;
     ...
     */
    public ChordifyCommand(ChordPart source, int startSlot, int stopSlot,
                                     boolean undoable, boolean play) {
        this(source, startSlot, stopSlot, play);
        this.undoable = undoable;
        Trace.log(2, "creating Chordify Command");
    }
    ...
    public void execute() 
    {
      ...
      source.setChord(stopSlot + 1, new Chord("C"));
      ...
      // {
        ...
        // source = source.chordify(startSlot, stopSlot);

        // oldSection = dest.extractSlots(startSlot, stopSlot);

        // dest.pasteOver(source, startSlot);
        
        // if( play && source instanceof ChordPart && ImproVisor.getPlay() )
        // {
